function [matShuffled] = TSPE_shuffling(sdf, d, neg_wins, co_wins, pos_wins, FLAG_NORM)
% Parameters:
...
%% SHUFFLING
Nshuffling=50;
matShuffled=zeros(NrC,NrC,Nshuffling);
clear u_mean mat l
for shuffling_time=1:Nshuffling
  
         %% shuffling
         u_0_shuffled = Shuffle(u_0, 1);
        ...
        %         CM(ind,:,:)=(mat(1+i:end,:)'*mat(1:end-i,:))./(r'*r)/NrS;
                ...
                % Correct form: 
                CM(ind,:,:)=(u_0(1+i:end,:)'*u_0_shuffled(1:end-i,:))./(r'*r)/NrS;
        ...
        %% Adjustment and looking for maximum at each delay time 
            ...
            matShuffled(:,:,shuffling_time)=CMres;
