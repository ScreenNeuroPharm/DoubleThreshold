function[h, FitParameters,Y,X,plotFittedData] = DistribDegreeExcInhAB_structural(Net, ConnRule)
...
idx=Y>mean(Y(find(X==max(X))))-30 & Y<250; 
X=X(idx);
Y=Y(idx);
idx=y_e>mean(y_e(find(x_e==max(x_e))))-6 & y_e<200;
x_e=x_e(idx);
y_e=y_e(idx);
idx=y_i>mean(y_i(find(x_i==max(x_i))))-6 & y_i<200;
x_i=x_i(idx);
y_i=y_i(idx);
end
