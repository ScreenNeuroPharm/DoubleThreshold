...
nexc=1;
ninh=2;
mexc=repmat(1.5:0.1:3,[1 16]);
minh=repelem(1.5:0.1:3,16);

[C,CC] = ThresholdMatrix2(ConnectivityMatrix);
cd(tempFolder)
...
% -------------------------- exc Nlink--------------------------------
figure
bar(mexc(1:16),mean(link_ms_exc(1:16,:),2));
pos_err = std(link_ms_exc(1:16,:),0,2);
neg_err = -std(link_ms_exc(1:16,:),0,2);
hold on 
er = errorbar(mexc(1:16),mean(link_ms_exc(1:16,:),2),pos_err,neg_err); 
...
er.LineStyle = 'none';  
er.LineWidth = 2;
plot(linspace(1.5,3,17),ones(1,17).*Nlinkexc(1),'--g','LineWidth',2)
legend('Mean','Std','Structural Target')
hold off
% -------------------------- inh Nlink--------------------------------
figure
bar(minh(1:16:end),mean(link_ms_inh(1:16:end,:),2));
pos_err = std(link_ms_inh(1:16:end,:),0,2);
neg_err = -std(link_ms_inh(1:16:end,:),0,2);
hold on 
er = errorbar(minh(1:16:end),mean(link_ms_inh(1:16:end,:),2),pos_err,neg_err); 
...
er.LineStyle = 'none';  
er.LineWidth = 2;
plot(linspace(1.5,3,17),ones(1,17).*Nlinkinh(1),'--g','LineWidth',2)
legend('Mean','Std','Structural Target')
hold off
%% ------------------------ plot TPR -------------------------------

figure
bar(mexc(1:16),mean(Cexc(1:16,:),2));
pos_err = std(Cexc(1:16,:),0,2);
neg_err = -std(Cexc(1:16,:),0,2);
hold on 
er = errorbar(mexc(1:16),mean(Cexc(1:16,:),2),pos_err,neg_err); 
...
% -------------------------- inh Nlink--------------------------------
figure
bar(minh(1:16:end),mean(Cinh(1:16:end,:),2));
pos_err = std(Cinh(1:16:end,:),0,2);
neg_err = -std(Cinh(1:16:end,:),0,2);
hold on 
er = errorbar(minh(1:16:end),mean(Cinh(1:16:end,:),2),pos_err,neg_err); 
...
%% --------------------------- Ratio exc/inh ------------------------
   ...
   C(:,:,3)=1;
   f=figure
   surf(mexc(1:16),minh(1:16:end),reshape(ratio_ms(:,1),[16 16])',C,'FaceAlpha',0.8)
   zlabel('EXC/INH')
   hold on
   
   C=zeros(16,16,3);
   C(:,:,1)=1;
   surf(mexc(1:16),minh(1:16:end),ones(16).*0.90,C,'FaceAlpha',0.3)
    xlabel('m_exc')
    ylabel('m_inh')
    zlabel('EXC/INH')
   ...
   C=zeros(16,16,3);
   C(:,:,2)=1;
   surf(mexc(1:16),minh(1:16:end),ones(16).*0.70,C,'FaceAlpha',0.3)
    xlabel('m_exc')
    ylabel('m_inh')
    zlabel('EXC/INH')
