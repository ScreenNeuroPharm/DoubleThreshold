...
%     saveas(h,'DegreeDistribution_Struct.fig','fig');
    ...
    [CC_meanstd,CC_cost,CC_bin_meanstd,CC_bin_cost] = ThresholdMatrixEvaluation(CCfolder,nexc,ninh,syn,1);
    ...
    n_0 = size(AD_bin_0,1);  % number of nodes
    m_0 = sum(sum(AD_bin_0)); %number of edges directed network
    [Lrand_0,CrandWS_0] = NullModel_L_C(n_0,m_0,100,1);
    [SW(2,k),Ccoeff(2,k),PL(2,k)] = small_world_ness2(AD_bin_0,Lrand_0,CrandWS_0,1); 
    ...
    for j=1:length(nexc)*2
       ...
       %  ----------------------SWI----------------------------------------
         ...
         [Lrand,CrandWS] = NullModel_L_C(n,m,100,1);
         [SW(j+2,k),Ccoeff(j+2,k),PL(j+2,k)] = small_world_ness2(tmp_bin,Lrand,CrandWS,1);     
         %---------------------Weight distribution-------------------------
          ...
          D(j,k)=rms(InhFunctProf - InhStructProf)/rms(InhStructProf);       % RMS
          %--------------------N links-------------------------------------
          Nlink(j+1,k)=length(find(tmp(tmp~=0)));
          %---------------------------TPR----------------------------------
          tempTPR=tmp_bin+AD_bin;
          tempFPR=tmp_bin-AD_bin;
          TRP(j,k)=length(find(tempTPR==2))./(length(find(tempFPR==1))+length(find(tempTPR==2)));
          %-----------------Degree distribution---------------------------
...
%%
    ...
    xticklabels({'MS 2','COST 2'})
    ylabel('RMS distance')
    saveas(fig,'Norm_profile_Distance','fig')
   
    fig=figure(2);
    boxplot(SW');
    xticklabels({'Struct_0','Struct','MS 2','COST 2'})
    title(strcat('SWI ( deg=',num2str(net_folders(i+2).name),')'));
    ylabel('SWI')
    saveas(fig,'SWI','fig')
    % ------------ANOVA SW---------------
   [p,tbl,stats]  = anova1(SW',{'Struct_0','Struct','MS','Cost'});
   [c,~,~,gnames] = multcompare(stats);
    
    fig=figure(5);
    boxplot(Nlink')
    xticklabels({'Struct','MS 2','COST 2'})
    ...
    boxplot(TRP')
    xticklabels({'MS 2','COST 2'})
    title(strcat('TPR (deg=',num2str(net_folders(i+2).name),')'));
    saveas(fig,'TPR','fig')
    ylabel('TPR')
    
   % ------------ANOVA TPR---------------
   [p,tbl,stats]  = anova1(TRP',{'MS','Cost'});
   [c,~,~,gnames] = multcompare(stats);
    %----------------------------Fitting R2--------------------------------
    fig=figure(10);
    tempBoxR2=zeros(5);
